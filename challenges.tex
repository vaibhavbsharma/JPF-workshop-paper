...
%
The primary benefit of implementing veritesting comes from its conversion of branches into disjunctions in a multipath region.
%
...
%
If there were no exit points encountered on any side of the branch, we considered this region as a pure multipath region and calculated its size in bytecode instructions.
%
...
%
The added \textit{return} statement creates an exit point, which causes the three-way branch on lines 5, 6 (and the newly added line 7) of Listing~\ref{lst:v_ex} to contain two exit points.
%
...
%
The same count going through nodes (3) and (4) gives us 6 instructions, and through node (3) to node (5) gives us 4 instructions.
%
The presence of the \textit{return} statement in node (5) prevents this region from being a pure multipath region.
%
\begin{figure}[]
...
%
We believe that creating multipath regions for these no-exit-point cases alone would provide a significant performance boost to SPF.
%
...
the number of multipath regions.
%
\input{engineering}
%
%
