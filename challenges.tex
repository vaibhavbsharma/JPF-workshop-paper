...
Java bytecode instructions) and number of occurences of multi-path
...
While dynamic dispatch and return statements are some factors which
should not be allowed to limit veritesting of Java, there exist other
research challenges which need to be addressed to push the veritesting
frontier further.
%
Heap allocations which occur within a veritesting region can be treated
as exit points but including them in our predicate construction require
...
consideration because the symbolic execution engine may need to set the
symbolic object up correctly.
%
